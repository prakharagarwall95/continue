...
export class OcaTokenManager {
  ...
  private static readonly TOKEN_CACHE_PATH = path.join(
    os.homedir(),
    ".oca",
    "token_cache.json",
  );
  private static readonly RENEW_TOKEN_BUFFER_SEC = 180; // 3 minutes
  private static readonly OCA_CONFIG_PATH = path.join(
    os.homedir(),
    ".oca",
    "config.json",
  );
  private static async getConfig() {
    ...
    this._config = {
      ...
      use_pkce:
        typeof cfg.use_pkce === "boolean" ? cfg.use_pkce : DEFAULT_USE_PKCE,
    };
    return this._config;
  }
  private static async saveToken(token: TokenSet) {
    ...
    await fs.writeFile(this.TOKEN_CACHE_PATH, JSON.stringify(toStore), {
      encoding: "utf-8",
    });
    ...
    try {
      ...
      if (os.platform() !== "win32") {
        ...
        if (mode !== 0o600) {
          ...
          throw new Error(
            "Token file permissions should be owner read and write",
          );
        }
      }
  ...
  private static async findAndBindAvailablePort(): Promise<{
    port: number;
    server: http.Server;
  }> {
  ...
  private static isAccessTokenValid(token: OcaStoredToken): boolean {
    ...
    return (
      ...
      now < token.expires_at - this.RENEW_TOKEN_BUFFER_SEC
    );
  }
  ...
  public static async refreshToken(): Promise<OcaStoredToken | null> {
    ...
    try {
      const refreshed: TokenSet = await client.refresh(
        this.cachedToken.refresh_token,
      );
  ...
  public static async getToken(): Promise<OcaStoredToken> {
    ...
    const tokenPromise = new Promise<OcaStoredToken>((resolve, reject) => {
      ...
      (async () => {
        ...
        reject(err); // Optionally reject the outer promise
      });
